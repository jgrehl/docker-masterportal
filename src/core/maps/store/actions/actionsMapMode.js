...
export default {
    ...
    async activateMap3D ({getters, dispatch, commit, rootState, state}) {
        ...
        if (getters.is3D) {
            ...
            api.map.olcsMap.handle3DEvents({scene: await map3D.getCesiumScene(), map3D: map3D, callback: (clickObject) => dispatch("clickEventCallback", Object.freeze(clickObject))});
        }
